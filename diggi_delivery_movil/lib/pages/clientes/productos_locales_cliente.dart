...
class ProductoLocalesCliente extends StatefulWidget {
  const ProductoLocalesCliente({Key key}) : super(key: key);
  static final routeName = "productoLocalesCliente";
  @override
  _ProductoLocalesClienteState createState() => _ProductoLocalesClienteState();
}

class _ProductoLocalesClienteState extends State<ProductoLocalesCliente> {
  LocalesProvider restaurantesProvider = LocalesProvider();
  ...
  Widget build(BuildContext context) {
    ...
    localesBloc.cargarProductosCliente();
    ...
    return Container(
      child: Scaffold(
        ...
        body: _crearGrid(context, localesBloc, size),
      ),
    );
  }

  Widget _crearGrid(BuildContext context, LocalesBloc localesBloc, Size size) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.all(5),
      child: StreamBuilder(
        // stream: restaurantesProvider.getPlatilloRestaurante(prefs.email),
        stream: localesBloc.productosClienteStream,
